KDsl.structure :bootstrap do
  microapp     = import(:k_zmen, :microapp)
  ...
  blueprint :bootstrap_cmdlet do
    ...
    # ADD EXE (executable)
    row 'exe/cmdlet_name', 'exe/k_zmen'

    # ADD EXT (extensions)
    row 'ext/cmdlet_name/cmdlet_name.c', 'ext/k_zmen/k_zmen.c'
    row 'ext/cmdlet_name/cmdlet_name.h', 'ext/k_zmen/k_zmen.h'
    row 'ext/cmdlet_name/extconf.rb', 'ext/k_zmen/extconf.rb'

    row 'bin/setup'
    row 'bin/console'

    # Setup CLI and command execution
    row 'lib/cmdlet_name.rb'              , 'lib/k_zmen.rb'
    row 'lib/cmdlet_name/version.rb'      , 'lib/k_zmen/version.rb', conflict: 'skip'
    row 'lib/cmdlet_name/cli.rb'          , 'lib/k_zmen/cli.rb'
    row 'lib/cmdlet_name/app.rb'          , 'lib/k_zmen/app.rb'
    row 'lib/cmdlet_name/command.rb'      , 'lib/k_zmen/command.rb'

    row '.keep'                           , 'lib/k_zmen/commands/.keep'
    row '.keep'                           , 'lib/k_zmen/templates/.keep'

    row 'spec/spec_helper.rb'
    row 'spec/cmdlet_name_spec.rb'        , 'spec/k_zmen_spec.rb'
    ...
    row 'Gemfile'
    row 'LICENSE.txt'
    row 'cmdlet_name.gemspec'               , 'k_zmen.gemspec'
    row 'Rakefile'
    row 'README.md'
    row 'Guardfile'
    row '.rubocop.yml'
  ...
  # actions do
  #   if is_bundle_gem == 1
  #     run_command 'bundle gem --coc --test=rspec --mit k_zmen', command_creates_top_folder: true
  #   end
  ...
  #   end
  #   if is_open_git == 1
  #     run_command "open -a 'Google Chrome' https://github.com//k_zmen"
  #   end
