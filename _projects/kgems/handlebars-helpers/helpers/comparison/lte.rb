KDsl.blueprint :lte do
  microapp = import(:handlebars_helpers, :microapp)

  s = settings do
    name                  'lte'
    description           "Lte: (less than or equal to) Block helper that renders a block if `a` is **less than or equal to** `b`. If an inverse block is specified it will be rendered when falsy."
    ...
    test_case(            { params: [
                              {name: 'lhs', type: 'String', sample_value: 1, description: 'left hand side value'},
                              {name: 'rhs', type: 'String', sample_value: 1, description: 'right hand side value'}
                            ],
