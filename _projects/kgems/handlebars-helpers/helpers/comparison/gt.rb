KDsl.blueprint :gt do
  microapp = import(:handlebars_helpers, :microapp)

  s = settings do
    name                  'gt'
    description           "Gt: (greater than) Block helper that renders a block if `a` is **greater than** `b`. If an inverse block is specified it will be rendered when falsy."
    ...
    test_case(            { params: [
                              {name: 'lhs', type: 'String', sample_value: 2, description: 'left hand side value'},
