KDsl.blueprint :eq do
  microapp = import(:handlebars_helpers, :microapp)

  s = settings do
    name                  'eq'
    description           "Eq: (equal) Block helper that renders a block if `a` is **equal to** `b`. If an inverse block is specified it will be rendered when falsy."
    result                "truthy value if left hand side equals right hand side"
    category              'comparison'
    ...
    # example_input_value      'the quick brown fox'
    # example_output_value      'xxxxxxxxxxxxxxxxxxx'
