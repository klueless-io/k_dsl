KDsl.blueprint :lt do
  microapp = import(:handlebars_helpers, :microapp)

  s = settings do
    name                  'lt'
    description           "Lt: (less than) Block helper that renders a block if `a` is **less than** `b`. If an inverse block is specified it will be rendered when falsy."
    ...
    test_case(            { params: [
                              ...
                              {name: 'rhs', type: 'String', sample_value: 2, description: 'right hand side value'}
                            ],
