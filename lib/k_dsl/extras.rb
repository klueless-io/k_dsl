  ...
  class Extras
    ...
    def initialize(key, *args, **options, &block)
      ...
      begin
        ...
        puts "Invalid code block in document_dsl during registration: #{k_key}"
        puts e.message
    ...
    def settings(key = nil, **options, &block)
      ...
      processor.modify_settings(modifiers, @meta_data[settings.k_key])
    ...
    def table(key = :table, &block)
      ...
      table = TableDsl.new(@meta_data, key, &block)
    ...
    def modify_data(data, opts)
      return if data.nil?

      # Find more support here
      # https://api.rubyonrails.org/classes/ActiveSupport/Inflector.html#method-i-underscore

      # # Can these two just go into the modifier config?
      # data.update(data) { |_k,v| v.upcase }                   if opts.dig(:upcase) == true
      # data.update(data) { |_k,v| v.downcase }                 if opts.dig(:downcase) == true

      # modify each key and do whatever you like to it
      modifiers = get_modifiers(opts.dig(:modifiers))

      return if modifiers.length.zero?

      if modifiers.present? && !modifiers.empty?
        modifiers.each do |modifier_class|
          modifier = modifier_class.new
