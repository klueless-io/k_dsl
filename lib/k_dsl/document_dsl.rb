...
module KDsl
  ...
  class DocumentDsl
    ...
    def initialize(key, *args, **options, &block)
      ...
      if block_given? # if !Klue.registering && block_given?
    ...
    def settings(key = nil, **_options, &block)
      # options ||= {}

      # opts = {}.merge(@options)  # Document Options
      #          .merge(options)   # Settings Options

      settings = KDsl::SettingsDsl.new(@meta_data, key, k_parent: self, &block)

      # process_setting_data_with_options(@meta_data[settings.k_key], opts)

      settings
    end

    def table(name = nil, &block) #, **options, &block)
      # # name ||= :rows
      # # @options = options ||= {}

      # # opts = {}
      # #   .merge(@options)  # Artifact Options
      # #   .merge(options)   # Settings Options

      # # Don't think I'm using this concept yet

      # rows = RowsDsl.new(@meta_data, name, &block)
    ...
    def process_setting_data_with_options(data, opts)
      # # return  if data.nil?
      
      # # Find more support here
      # # https://api.rubyonrails.org/classes/ActiveSupport/Inflector.html#method-i-underscore

      # data.update(data) { |_k,v| v.upcase }                   if opts.dig(:upcase) == true
      # data.update(data) { |_k,v| v.downcase }                 if opts.dig(:downcase) == true

      # # Visit each key and do whatever you like to it
      # visitors = opts.dig(:visitors)

      # if visitors.present? && !visitors.empty?
      #   visitors.each do |visitor_class|
