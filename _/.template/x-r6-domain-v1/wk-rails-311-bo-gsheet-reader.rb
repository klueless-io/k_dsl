module Gsheet

  module Reader

    # Read {{camelU settings.Model}} shaped data from a google spreadsheet
    #
    # @example
    #
    # gsheet = {{camelU settings.Model}}Gsheet.new()
    # table = gsheet.read
    class Gs{{camelU settings.Model}}Reader < BaseReader

      # Open a connection to a worksheet within a google spreadsheet
      #
      # @param [string] spreadsheet_name The name of the spreadsheet to open
      # @param [string] worksheet_name The name of the worksheet to open on the spreadsheet
      # @param [string] config_key The key to the Google configuration that will be used
      def initialize(spreadsheet_name, worksheet_name = '{{dashify settings.Model}}', config_key = nil)
        super(spreadsheet_name, worksheet_name, config_key)
      end

      # Map raw spreadsheet data to Gs{{camelU settings.Model}}
      def map_row(row)
        return Gs{{camelU settings.Model}}.new(row)
      end

    end

  end

end
