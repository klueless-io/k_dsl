...
class PgUtil
  ...
  def self.execute_sql_in_file(file, key_values = [])
    ...
    return self.execute_sql(sql, key_values)
  ...
  def self.execute_sql(sql, key_values = [])
    ...
    return ActiveRecord::Base.connection.execute(sanitized_sql)
  end
  ...
  def self.sanitize_sql(sql, key_values = [])
    ...
    # See: http://api.rubyonrails.org/classes/ActiveRecord/Sanitization/ClassMethods.html
    return Arel.sql(ActiveRecord::Base.send(:sanitize_sql, [*sql, *key_values]))
  end
  ...
  def self.index_list(table_name)
    ...
    return ActiveRecord::Base.connection.execute(sanitized_sql).map {|row|row['index_name']}
  ...
  def self.index_exists(table_name, index_name)
    ...
    return ActiveRecord::Base.connection.execute(sanitized_sql).first["count"] == 1
  ...
  def self.check_index_integrity(table_name, expected_indexes)
    ...
    expected_indexes.each do |expected_index|
      result = result && indexes.include?(expected_index)

      if !indexes.include?(expected_index)
        L.kv 'Missing Index', expected_index
      end
