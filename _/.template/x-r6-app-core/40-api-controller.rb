class ApiController < ApplicationController
  ...
  # A default sample is generally specified which can be modified or added too
	...
	def build_sample_endpoints(type, main_key: 'name')
		...
		port = request.port
    return [
            {key: :index          , type: 'GET'   , description: "Get a full list of #{type_plural}", url: "http://localhost:#{port}/api/v1/#{type_snake_plural}"},
            {key: :index_filter   , type: 'GET'   , description: "Get a full list of #{type_plural} with filter, sorting and pagination", url: "http://localhost:#{port}/api/v1/#{type_snake_plural}" + '?options={"page":{"active":true,"no":1,"size":50},"filter":{"search":""},"sort":[{"field":"' + main_key+ '","sort":"asc"}]}'},
            {key: :show           , type: 'GET'   , description: "Get a single, #{type}", url: "http://localhost:#{port}/api/v1/#{type_snake_plural}/1"},
            {key: :show_failure   , type: 'GET'   , description: "Get a single #{type}, with invalid ID", url: "http://localhost:#{port}/api/v1/#{type_snake_plural}/10000069"},
            {key: :multi          , type: 'GET'   , description: "Get multiple #{type_plural} by ID", url: "http://localhost:#{port}/api/v1/#{type_snake_plural}/multi?ids=1,2"},
            {key: :create         , type: 'POST'  , description: "Create #{type}", url: "http://localhost:#{port}/api/v1/#{type_snake_plural}"},
            {key: :update         , type: 'PUT'   , description: "Update #{type}", url: "http://localhost:#{port}/api/v1/#{type_snake_plural}/1"},
            {key: :destroy        , type: 'DELETE', description: "Destroy #{type}", url: "http://localhost:#{port}/api/v1/#{type_snake_plural}/id"}
        ]
  end
  ...
  def build_data()
	...
	# Helper to render a success based respone on a single row, used by show, create and update
