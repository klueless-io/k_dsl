...
# rails generate model {{camel entity.model_name}} {{#each entity.columns}}{{#if (eq db_type 'primary_key')}}{{else}}{{snake this.name}}:{{this.db_type}} {{/if}}{{/each}} --no-test-framework
class Create{{camel entity.name}} < ActiveRecord::Migration[6.1]
  def change
