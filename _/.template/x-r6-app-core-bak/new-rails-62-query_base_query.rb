require 'query/query_page_do.rb'
require 'query/query_sort_do.rb'
require 'query/query_option_do.rb'
...
module Query
  class BaseQuery
    ...
    def add_suggest_ilike(field, value)
      return @relation.where("#{field} ilike ?", "%#{value}%")
    end
    ...
    def add_suggest_ilike_all(field, values)
      return @relation.where("#{field} ilike all (array[?])", values.map { |map| "%#{map}%" })
    end
    ...
    def add_suggest_ilike_any(field, values)
      return @relation.where("#{field} ilike any (array[?])", values.map { |map| "%#{map}%" })
    end
    ...
    def process_options(options)
      ...
      optionsHash = Util::parse_as_hash(options)

      @options = QueryOptionDo.new(optionsHash)
